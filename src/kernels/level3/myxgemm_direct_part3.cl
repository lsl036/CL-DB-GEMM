...
INLINE_FUNC void MyXgemmDirect(const int kSizeM, const int kSizeN, const int kSizeK,
      ...
      for (int pwi = 0; pwi < WGD; pwi += KWID) {
        ...
        for (int _pit = 0; _pit < KWID; _pit += 1) {
          ...
          for (int _ni = 0; _ni < NWID; _ni += 1) {
            #pragma unroll
            for (int _mi = 0; _mi < MWID; _mi += 1) {
              MultiplyAdd(cpd[_ni * MWID + _mi], apd[_mi], bpd[_ni]); //标量
            }
          }
        }
      }
...
void XgemmDirectNN(const int kSizeM, const int kSizeN, const int kSizeK,
  ...
  MyXgemmDirect(kSizeM, kSizeN, kSizeK, arg_alpha, arg_beta,
