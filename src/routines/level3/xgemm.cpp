...
void Xgemm<T>::DoGemm(const Layout layout,
  ...
  const auto do_gemm_direct = UseDirectKernel(m, n, k, db_["XGEMM_MIN_INDIRECT_SIZE"]); // m*n*k < size*size*size
...
void Xgemm<T>::GemmIndirect(const size_t m, const size_t n, const size_t k,
  ...
  auto a_no_temp = NoTempBuffer(a_one, a_one_i, a_two, a_two_i, a_ld, a_offset, a_do_transpose, a_conjugate); //A no temp=1 列主序
  auto b_no_temp = NoTempBuffer(b_one, b_one_i, b_two, b_two_i, b_ld, b_offset, b_do_transpose, b_conjugate); //B no temp=0 列主序
...
// Constructor: forwards to base class constructor
template <typename T>
MyXgemm<T>::MyXgemm(Queue &queue, EventPointer event, const std::string &name):
    Routine(queue, event, name,       // name = "MyGEMM"
            {"Copy","Pad","Transpose","Padtranspose","MyXgemm","MyXgemmDirect","MyGemmRoutine"},
            PrecisionValue<T>(), {}, {
    #include "../../kernels/level3/level3.opencl"
    #include "../../kernels/level3/copy_fast.opencl"
    #include "../../kernels/level3/copy_pad.opencl"
    ...
    #include "../../kernels/level3/myxgemm_direct_part1.cl"
    #include "../../kernels/level3/myxgemm_direct_part2.cl"
    #include "../../kernels/level3/myxgemm_direct_part3.cl"
    , // separated in multiple parts to prevent C1091 in MSVC 2013

    #include "../../kernels/level3/myxgemm_part1.cl"
    #include "../../kernels/level3/myxgemm_part2.cl"
    , // separated in multiple parts to prevent C1091 in MSVC 2013

    #include "../../kernels/level3/myxgemm_part3.cl"
    #include "../../kernels/level3/myxgemm_part4.cl"
    }) {
}

// The main routine
template <typename T>
void MyXgemm<T>::DoMyGemm(const Layout layout,
                      const Transpose a_transpose, const Transpose b_transpose,
                      const size_t m, const size_t n, const size_t k,
                      const T alpha,
                      const Buffer<T> &a_buffer, const size_t a_offset, const size_t a_ld,
  ...
  // Selects which version of GEMM to run
  if (do_gemm_direct) { // for small sizes (single kernel)
    MyGemmDirect(m, n, k, alpha,
               a_buffer, a_offset, a_ld, b_buffer, b_offset, b_ld, beta,
               c_buffer, c_offset, c_ld,
               a_do_transpose, b_do_transpose, c_do_transpose, a_conjugate, b_conjugate);
  }
  else { // for larger sizes (pre/post-processing plus a very fast kernel)
    MyGemmIndirect(m, n, k, alpha,
                 a_buffer, a_offset, a_ld, b_buffer, b_offset, b_ld, beta,
                 c_buffer, c_offset, c_ld,
                 a_do_transpose, b_do_transpose, c_do_transpose, a_conjugate, b_conjugate,
                 a_one, a_two, b_one, b_two, c_one, c_two,
...
template <typename T>
void MyXgemm<T>::MyGemmIndirect(const size_t m, const size_t n, const size_t k,
                            const T alpha,
                            const Buffer<T> &a_buffer, const size_t a_offset, const size_t a_ld,
                            const Buffer<T> &b_buffer, const size_t b_offset, const size_t b_ld,
                            const T beta,
  ...
  // Retrieves the Xgemm kernel from the compiled binary
  auto kernel = Kernel(program_, "MyXgemm");
...
// The direct version of GEMM, requiring just one kernel, no pre or post-processing kernels.
template <typename T>
void MyXgemm<T>::MyGemmDirect(const size_t m, const size_t n, const size_t k,
                          const T alpha,
                          const Buffer<T> &a_buffer, const size_t a_offset, const size_t a_ld,
                          const Buffer<T> &b_buffer, const size_t b_offset, const size_t b_ld,
                          const T beta,
...
template class MyXgemm<half>;
template class MyXgemm<float>;
template class MyXgemm<double>;
template class MyXgemm<float2>;
template class MyXgemm<double2>;
// ====================================================================================
} // namespace clblast
