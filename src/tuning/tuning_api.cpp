...
// tuning MyXgemm
template <typename T>
StatusCode TuneMyXgemm(RawCommandQueue * queue, const size_t m, const size_t n, const size_t k,
                     const double fraction, std::unordered_map<std::string,size_t> &parameters) {
  auto args = Arguments<T>(); args.fraction = fraction; args.m = m; args.n = n; args.k = k;
  auto queue_cpp = Queue(*queue);
  auto status = TunerAPI<T>(queue_cpp, args, 2, MyXgemmGetTunerDefaults, MyXgemmGetTunerSettings<T>, 
                            MyXgemmTestValidArguments<T>, MyXgemmSetConstraints, MyXgemmComputeLocalMemSize<T>, MyXgemmSetArguments<T>, parameters);
  if (status != StatusCode::kSuccess) { return status; }
  // return TunerAPI<T>(queue_cpp, args, 12, MyXgemmGetTunerDefaults, MyXgemmGetTunerSettings<T>,
  //                    MyXgemmTestValidArguments<T>, MyXgemmSetConstraints, MyXgemmComputeLocalMemSize<T>, MyXgemmSetArguments<T>, parameters);
  return TunerAPI<T>(queue_cpp, args, 2, MyXgemmGetTunerDefaults, MyXgemmGetTunerSettings<T>,
                     MyXgemmTestValidArguments<T>, MyXgemmSetConstraints, MyXgemmComputeLocalMemSize<T>, MyXgemmSetArguments<T>, parameters);
}
template StatusCode PUBLIC_API TuneMyXgemm<half>(RawCommandQueue*, const size_t, const size_t, const size_t, const double, std::unordered_map<std::string,size_t>&);
template StatusCode PUBLIC_API TuneMyXgemm<float>(RawCommandQueue*, const size_t, const size_t, const size_t, const double, std::unordered_map<std::string,size_t>&);
template StatusCode PUBLIC_API TuneMyXgemm<double>(RawCommandQueue*, const size_t, const size_t, const size_t, const double, std::unordered_map<std::string,size_t>&);
template StatusCode PUBLIC_API TuneMyXgemm<float2>(RawCommandQueue*, const size_t, const size_t, const size_t, const double, std::unordered_map<std::string,size_t>&);
template StatusCode PUBLIC_API TuneMyXgemm<double2>(RawCommandQueue*, const size_t, const size_t, const size_t, const double, std::unordered_map<std::string,size_t>&);
/**  Added by myself */
