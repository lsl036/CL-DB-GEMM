...
//
// This file uses the auto-tuner to tune the myxgemm OpenCL kernels. There are multiple variations:
...
// 内核调优基础设置
TunerDefaults MyXgemmGetTunerDefaults(const int V) {
...
template <typename T>
TunerSettings MyXgemmGetTunerSettings(const int V, const Arguments<T> &args) {
  auto settings = TunerSettings();

  // Identification of the kernel
  settings.kernel_family = "myxgemm_" + ToString(V);
  settings.kernel_name = "MyXgemm";
  // settings.sources = (V == 11 || V == 12) ? "#define GEMMK 1" : "#define GEMMK 0";
  // 暂时没有写 GEMMK = 1 的
  settings.sources = "#define GEMMK 0" ;
  settings.sources +=
#include "../src/kernels/level3/myxgemm_part1.cl"
#include "../src/kernels/level3/myxgemm_part2.cl"
  ;
  settings.sources +=
#include "../src/kernels/level3/myxgemm_part3.cl"
#include "../src/kernels/level3/myxgemm_part4.cl"
  ;
...
template <typename T>
void MyXgemmTestValidArguments(const int V, const Arguments<T> &args) {
  const auto mwg_max = (V == 1 || V == 11) ? 64 : 128;
  const auto nwg_max = (V == 1 || V == 11) ? 64 : 128;

  // M、N 必须是 mwg的整数倍
  if (!IsMultiple(args.m, mwg_max)) {
    throw std::runtime_error("'MyXgemm' kernel requires 'm' to be a multiple of MWG (max " + ToString(mwg_max) + ")");
  }
  if (!IsMultiple(args.n, nwg_max)) {
    throw std::runtime_error("'MyXgemm' kernel requires 'n' to be a multiple of NWG (max " + ToString(nwg_max) + ")");
  }
}
std::vector<Constraint> MyXgemmSetConstraints(const int V) {
  auto constraints = std::vector<Constraint>();
  auto IsEqual = [] (std::vector<size_t> v) { return v[0] == v[1]; };
  auto MultipleOfX = [] (std::vector<size_t> v) { return IsMultiple(v[0], v[1]); }; // (v[0]/ v[1]) * v[1] == v[0] ? true : false;
...
template <typename T>
LocalMemSizeInfo MyXgemmComputeLocalMemSize(const int) {
  return {
...
template <typename T>
void MyXgemmSetArguments(const int, Kernel &kernel, const Arguments<T> &args, std::vector<Buffer<T>>& buffers) {
