...
void Tuner(int argc, char* argv[], const int V,
  ...
  const TunerDefaults defaults = GetTunerDefaults(V); // 初始化最基础的设定, 测试用例的size、循环次数等
  ...
  printf("%s\n", help.c_str());
  const TunerSettings settings = GetTunerSettings(V, args); // 初始化kernel的设定,包含kernel名称, kernel的cl文件, buffer size, 相应的参数组合等

  // Tests validity of the given arguments
  TestValidArguments(V, args); // 验证给定参数的有效性,主要是M、N必须是分块的整数倍
  ...
  // Creates input buffers with random data. Adds a 'canary' region to detect buffer overflows. kCanarySize=127
  const auto buffer_sizes = std::vector<size_t>{  // 稍微放大一点buffer_size
  ...
  std::uniform_real_distribution<double> dist(kTestDataLowerLimit, kTestDataUpperLimit); // [-2,2) 均匀分布中生成随机浮点数
  ...
  for (const auto size : buffer_sizes) {
    ...
    PopulateVector(host_buffer, mt, dist);    //用[-2,2)的随机数据填充向量
  ...
  //   printf( "\nC[0] = %f\n", reference_buffers[id][0] );
  //   printf( "C[1] = %f\n", reference_buffers[id][1] );
  //   printf( "C[2] = %f\n", reference_buffers[id][2] );
  //   printf( "C[3] = %f\n", reference_buffers[id][3] );
  //   printf( "C[4] = %f\n", reference_buffers[id][4] );
  // }
  ...
  // Starts the tuning process 开始调优过程
  auto results = std::vector<TuningResult>();
  for (auto config_id = size_t{0}; config_id < configurations.size(); ++config_id) {
    // 取一个configuration 进行编译运行
    try {
      // Configuration 是map<std::string, size_t> |  configurations: vector<Configuration>
      auto configuration = configurations[config_id];
      printf("| %4zu | %5zu |", config_id + 1, configurations.size());
      for (const auto& parameter : settings.parameters) {     // parameters: vector<Parameter>   Parameter: pair<string,vector>
        printf("%5zu", configuration.at(parameter.first));    // 使用parameter的 string 作为key来输出可行 configuration中的值
      }
      printf(" |");
      ...
      for (const auto &parameter : configuration) { // 给kernel加参数定义
      ...
      printf(" %6.1lf |", settings.metric_amount / (time_ms * 1.0e6));  // Gflops
