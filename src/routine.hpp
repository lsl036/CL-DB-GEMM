...
class Routine {
 public:
  ...
  static void InitDatabase(const Device &device, const std::vector<std::string> &kernel_names, //{"Copy","Pad","Transpose","Padtranspose","MyXgemm","MyXgemmDirect","MyGemmRoutine"}
    ...
    for (const auto &kernel_name : kernel_names) {
      ...
      bool has_db;
      // 逐步搜索 kernel_names 中的每个 kernel_name; Instance() 获取kernel中的一个参数对。 
      //db(kernel_name) 是 Databases({"Copy","Pad","Transpose","Padtranspose","MyXgemm","MyXgemmDirect","MyGemmRoutine"}), 对应其string的 Database
      db(kernel_name) = DatabaseCache::Instance().Get(DatabaseKeyRef{platform_id, device(), precision, kernel_name}, &has_db);
      if (has_db) { continue; }
      ...
      DatabaseCache::Instance().Store(DatabaseKey{platform_id, device(), precision, kernel_name}, Database{db(kernel_name)});
    }
  }
  ...
  static const std::vector<std::string> routines_mygemm;
