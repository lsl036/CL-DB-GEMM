...
clblast::TunerSettings BayesianSettings(const int V, const clblast::Arguments<half> &args, Hypers_GEMM hyper_para){
...
int main(int argc, char** argv) {
  ...
  auto args = clblast::Arguments<half>{};
  ...
  args.precision   = clblast::GetArgument(command_line_args, help, clblast::kArgPrecision, clblast::Precision::kHalf);
  for (auto &o: defaults.options) {
    ...
    if (o == clblast::kArgAlpha)      { args.alpha       = clblast::GetArgument(command_line_args, help, clblast::kArgAlpha, clblast::GetScalar<half>()); }
    if (o == clblast::kArgBeta)       { args.beta        = clblast::GetArgument(command_line_args, help, clblast::kArgBeta, clblast::GetScalar<half>()); }
  ...
  if (!clblast::PrecisionSupported<half>(device)) {
  ...
  // 创建输入矩阵
  auto source_buffers = std::vector<std::vector<half>>();
  auto reference_buffers = std::vector<std::vector<half>>();
  auto result_buffers = std::vector<std::vector<half>>();
  auto device_buffers = std::vector<clblast::Buffer<half>>();

  // 随机数赋值
  for (const auto size : buffer_sizes) {
    auto host_buffer = std::vector<half>(size);
    clblast::PopulateVector(host_buffer, mt, dist);    //用[-2,2)的随机数据填充向量
    source_buffers.push_back(host_buffer);
    reference_buffers.push_back(std::vector<half>(size));
    result_buffers.push_back(std::vector<half>(size));
    device_buffers.push_back(clblast::Buffer<half>(context, size));
  }
#ifdef DEBUG
  ...
  auto configurations = SetConfigurations(device, settings.parameters, settings.local_size,
                                          ...
                                          clblast::XgemmSetConstraints(V), clblast::XgemmComputeLocalMemSize<half>(V));
#ifdef DEBUG
  ...
  try {
    ...
    clblast::XgemmSetArguments<half>(V, kernel, args, device_buffers);
