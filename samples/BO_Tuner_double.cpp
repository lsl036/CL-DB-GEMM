...
clblast::TunerSettings BayesianSettings(const int V, const clblast::Arguments<double> &args, Hypers_GEMM hyper_para){
...
int main(int argc, char** argv) {
  ...
  auto args = clblast::Arguments<double>{};
  ...
  args.precision   = clblast::GetArgument(command_line_args, help, clblast::kArgPrecision, clblast::Precision::kDouble);
  for (auto &o: defaults.options) {
    ...
    if (o == clblast::kArgAlpha)      { args.alpha       = clblast::GetArgument(command_line_args, help, clblast::kArgAlpha, clblast::GetScalar<double>()); }
    if (o == clblast::kArgBeta)       { args.beta        = clblast::GetArgument(command_line_args, help, clblast::kArgBeta, clblast::GetScalar<double>()); }
  ...
  if (!clblast::PrecisionSupported<double>(device)) {
  ...
  // 创建输入矩阵
  auto source_buffers = std::vector<std::vector<double>>();
  auto reference_buffers = std::vector<std::vector<double>>();
  auto result_buffers = std::vector<std::vector<double>>();
  auto device_buffers = std::vector<clblast::Buffer<double>>();

  // 随机数赋值
  for (const auto size : buffer_sizes) {
    auto host_buffer = std::vector<double>(size);
    clblast::PopulateVector(host_buffer, mt, dist);    //用[-2,2)的随机数据填充向量
    source_buffers.push_back(host_buffer);
    reference_buffers.push_back(std::vector<double>(size));
    result_buffers.push_back(std::vector<double>(size));
    device_buffers.push_back(clblast::Buffer<double>(context, size));
  }
#ifdef DEBUG
  ...
  auto configurations = SetConfigurations(device, settings.parameters, settings.local_size,
                                          ...
                                          clblast::XgemmSetConstraints(V), clblast::XgemmComputeLocalMemSize<double>(V));
#ifdef DEBUG
  ...
  try {
    ...
    clblast::XgemmSetArguments<double>(V, kernel, args, device_buffers);
