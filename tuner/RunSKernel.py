...
def Run_Xgemm_Kernel(parameters):
    ...
    try:
        # Single precision
        cmd = ['./clblast_sample_BO_Tuner_single', '-platform', '2', '-precision', '32', '-GEMMK', str(parameters[0]), '-KREG', str(parameters[1]), '-KWG', str(parameters[2]), '-KWI', str(parameters[3]), '-MDIMA', str(parameters[4]), '-MDIMC', str(parameters[5]), '-MWG', str(parameters[6]), '-NDIMB', str(parameters[7]), '-NDIMC', str(parameters[8]), '-NWG', str(parameters[9]), '-SA', str(parameters[10]), '-SB', str(parameters[11]), '-STRM', str(parameters[12]), '-STRN', str(parameters[13]), '-VWM', str(parameters[14]), '-VWN', str(parameters[15])]
